...
#MTU_SIZES=(1480)
#MTU_WITHOUT_IPIP=(1500)

HTTP_SERVER_HOSTNAME="net-arm-thunderx2-04"
HTTP_SERVER_ACCESSIP=192.168.26.1
HTTP_SERVER2_ACCESSIP=192.168.26.2
...
restart_calico()
{
  ...
  # Stop calico in K8s
  kubectl delete -f calico.yaml || true
...
do
  ...
  WRK_TEST_CMD="wrk -t5 -c10 -d30s http://${HTTP_SERVER_ACCESSIP}:${svcNodePort}/files/${file}"
  echo $WRK_TEST_CMD
...
do
  ...
  WRK_TEST_CMD="/usr/bin/wrk -t5 -c10 -d30s http://${NGINX_POD_IP1}/files/${file}"
  echo $WRK_TEST_CMD
  TEST_TYPE="host2pod"
...
do
  ...
  WRK_TEST_CMD="wrk -t5 -c10 -d30s http://${NGINX_POD_IP1}/files/${file}"
  echo $WRK_TEST_CMD
  TEST_TYPE="pod2pod"
  file="./test_result/wrk_${TEST_TYPE}_${use_ipip}_${MTU}.txt"
...
do
  ...
  WRK_TEST_CMD="wrk -t5 -c10 -d30s http://${svcIP}/files/${file}"
  echo $WRK_TEST_CMD
  TEST_TYPE="host2svc"
  echo $WRK_TEST_CMD >> ./test_result/wrk_${TEST_TYPE}_${use_ipip}_${MTU}.txt
  sshpass -p ${HOST_USER_PWD} ssh -o StrictHostKeyChecking=no ${HOST_USER}@${HTTP_CLIENT_HOSTNAME} ${WRK_TEST_CMD} >> ./test_result/wrk_${TEST_TYPE}_${use_ipip}_${MTU}.txt
